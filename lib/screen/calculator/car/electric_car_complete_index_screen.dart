...
class ElectricCarCompleteIndexScreen extends StatefulWidget {
  ...
  ElectricCarCompleteIndexScreen({super.key, this.title, this.bikeCC, this.data});

  @override
  State<ElectricCarCompleteIndexScreen> createState() => _ElectricCarCompleteIndexScreenState();
}

class _ElectricCarCompleteIndexScreenState extends State<ElectricCarCompleteIndexScreen> {
  double idv = 0;
  ...
  initFun(){
    setState(() {
      liability = two.electricCarCompleteLiabilityTP(widget.data?.tpTerm, widget.data?.kilowatt);
      ...
      vehicleBasicRate = two.electricCarBasicRate(widget.data?.ageVehicle,widget.data?.zone, widget.data?.kilowatt);
      ...
      totalB = zeroDep + rsa + tyreCover + ncb + enginePro + returnInvoice +
