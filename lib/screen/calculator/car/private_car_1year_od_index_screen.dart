...
class PrivateCar1YearODIndexScreen extends StatefulWidget {
  String? title;
  String? bikeCC;
  TwoWheelerPremiumModel? data;
  PrivateCar1YearODIndexScreen({super.key, this.title, this.bikeCC, this.data});

  @override
  State<PrivateCar1YearODIndexScreen> createState() => _PrivateCar1YearODIndexScreenState();
}

class _PrivateCar1YearODIndexScreenState extends State<PrivateCar1YearODIndexScreen> {
  double idv = 0;
  ...
  initFun(){
    setState(() {
      ...
      if(widget.data?.cngKits == "Yes"){
        cngExt = basicVehicle * 4 / 100;
      }
      basODPrime = (vehicleBasicRate * double.parse(widget.data!.currentIDV.toString()) / 100) + cngExt;
