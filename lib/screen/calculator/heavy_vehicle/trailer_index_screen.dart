...
class TrailerIndexScreen extends StatefulWidget {
  String? title;
  String? bikeCC;
  TwoWheelerPremiumModel? data;
  TrailerIndexScreen({super.key, this.title, this.bikeCC, this.data});

  @override
  State<TrailerIndexScreen> createState() => _TrailerIndexScreenState();
}

class _TrailerIndexScreenState extends State<TrailerIndexScreen> {
  double idv = 0;
  ...
  initFun(){
    setState(() {
      liability = double.parse(widget.bikeCC.toString()) * double.parse(widget.data!.noTrailers.toString());
      idv = double.parse(widget.data!.currentIDV.toString());
      vehicleBasicRate = 1.08;//0.54;
      if(widget.data?.vehicleType == "Agriculture Tractors upto 6 HP"){
        liability = 910 * double.parse(widget.data!.noTrailers.toString());
        vehicleBasicRate = 0.45;
      }
      basicVehicle = (vehicleBasicRate * double.parse(widget.data!.currentIDV.toString())) / 100 *  double.parse(widget.data!.noTrailers.toString());
      ...
      GST = (totalA + double.parse(widget.data!.paOwnerDriver.toString())
          + double.parse(widget.data!.llPaidDriver.toString()) + double.parse(widget.data!.llEmpPayed.toString()))* 18 / 100;
