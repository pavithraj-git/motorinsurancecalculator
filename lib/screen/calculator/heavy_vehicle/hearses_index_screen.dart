...
class HearsesIndexScreen extends StatefulWidget {
  ...
  HearsesIndexScreen({super.key, this.title, this.bikeCC, this.data});

  @override
  State<HearsesIndexScreen> createState() => _HearsesIndexScreenState();
}

class _HearsesIndexScreenState extends State<HearsesIndexScreen> {
  double idv = 0;
  ...
  initFun(){
    setState(() {
      ...
      totalB = (double.parse(liability.toString()) + double.parse(widget.data!.paOwnerDriver.toString())
          ...
          double.parse(widget.data!.llEmpPayed.toString())+ llPassenger) - tppd;
